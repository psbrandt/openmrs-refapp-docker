...
docker tag `docker images -q openmrsrefappdocker_openmrs-refapp-tomcat` psbrandt-docker-openmrs-refapp-alpine.bintray.io/tomcat:$TRAVIS_TAG

# Push the tomcat image
docker push psbrandt-docker-openmrs-refapp-alpine.bintray.io/tomcat:$TRAVIS_TAG

# Tag the mysql image
docker tag `docker images -q openmrsrefappdocker_openmrs-refapp-mysql` psbrandt-docker-openmrs-refapp-alpine.bintray.io/mysql:$TRAVIS_TAG
